...
![Target tải rev.sh từ HTTP server của attacker](images/revsh-download.png)
...
![Exploit chạy thành công — request timed out vì reverse shell đã kết nối](images/exploit-ssh.png)

### 2.5 Shell as `node`

Reverse shell kết nối thành công!

![Reverse shell — whoami: node, listing /opt/reactor-app](images/reverse-shell.png)
...
![Enum: .env, SQLite database, /etc/passwd, users table](images/sqlite-enum.png)
...
![Privilege Escalation — node inspect kết nối thành công, đọc root.txt](images/privesc-inspector.png)
